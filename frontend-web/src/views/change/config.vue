...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        label="币种"
        :width="getWidth('currency_name', 150)"
        prop="currency_name"
        align="center"
      >
        <template slot-scope="scope">
          <span style="color: lightseagreen; font-size: 150%">
            {{ scope.row.currency_name }}
          </span>
        </template>
      </el-table-column>
      <!-- <el-table-column label="交易对" width="150" align="center">
        <template slot-scope="scope">
          <span style="color: lightseagreen; font-size: 150%">
            {{ scope.row.symbol }}
          </span>
      ...
      <el-table-column
        label="平台"
        :width="getWidth('platform_text', 150)"
        prop="platform_text"
        align="center"
      >
        <template slot-scope="scope">
      ...
      <!-- <el-table-column width="300" label="备注" prop="remark">
        <template slot-scope="scope">
          <span
      ...
      <el-table-column
        ...
        prop="block_status"
        :width="getWidth('block_status', 70)"
      >
...
<script>
import { getChangeConfig, getPlatformList } from "@/api/table";
import {
  changeBlockId,
  changeBlockIdBatch,
  updateBlockRemark,
...
export default {
  ...
  methods: {
    getWidth(prop, defaultWidth) {
      const saved = localStorage.getItem(`diff_change_table_col_${prop}_width`);
      return saved ? parseInt(saved) : defaultWidth;
    },
    handleHeaderDragend(newWidth, oldWidth, column) {
      localStorage.setItem(
        `diff_change_table_col_${column.property}_width`,
        newWidth
      );
    },
    handleSwitchBatch(is_delete) {
      ...
      changeBlockIdBatch({ id: ids.join(","), is_delete: is_delete }).then(
        (res) => {
          if (res.code === 200) {
    ...
    async getTopics() {
      this.loading = true;
      const res = await getChangeConfig(this.query);
      this.list = res.data;
      this.loading = false;
    },
    handleFilter() {
