...
export default {
  ...
  data() {
    return {
      ...
      platformList: [],
      showAll: !(
        localStorage.getItem("change_left_search_box_show_all") == "false" ||
        localStorage.getItem("change_left_search_box_show_all") == null
      ),
      query: {
        direction: 1,
        order: "",
  ...
  methods: {
    ...
    getWidth(prop, defaultWidth) {
      const saved = localStorage.getItem(`diff_left_table_col_${prop}_width`);
      return saved ? parseInt(saved) : defaultWidth;
    },
    handleHeaderDragend(newWidth, oldWidth, column) {
      localStorage.setItem(
        `diff_left_table_col_${column.property}_width`,
        newWidth
      );
    },
    changeSecond() {
