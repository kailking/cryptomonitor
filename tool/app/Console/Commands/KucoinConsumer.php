...
class KucoinConsumer extends Command
{
    protected $signature = 'start_kucoin_consumer';
    protected $description = '消费kucoin队列';
    ...
    public function handle()
    {
        ...
        $platform = 8;
        while(true){
            $symbol = $redis->lPop('kucoin_symbol_list');
            if (!$symbol) {
                //                    echo '开始写锁';
                //拿写锁
                $lock = $redis->setnx('write_kucoin_lock', 1);
                $redis->expire('write_kucoin_lock', 3);
                if ($lock) {
                    $match = CurrencyMatch::where('is_kucoin', 1)->where('is_enabled', 1)->get();
                    foreach ($match as $m) {
                        $redis->rPush('kucoin_symbol_list', sprintf('%s%s', $m->currency_name, $m->quote_name));
                    }
                    $redis->del('write_kucoin_lock');
                }else{
                    sleep(3);
                    continue;
                }
