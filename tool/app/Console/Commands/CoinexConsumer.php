...
class CoinexConsumer extends Command
{
    protected $signature = 'start_coinex_consumer';
    protected $description = '消费coinex队列';
    ...
    public function handle()
    {
        ...
        $platform = 9;
        while (true) {
            $symbol = $redis->lPop('coinex_symbol_list');
            if (!$symbol) {
                //拿写锁
                $lock = $redis->setnx('write_coinex_lock', 1);
                $redis->expire('write_coinex_lock', 3);
                if ($lock) {
                    $match = CurrencyMatch::where('is_coinex', 1)->where('is_enabled', 1)->get();
                    foreach ($match as $m) {
                        $redis->rPush('coinex_symbol_list', sprintf('%s%s', $m->currency_name, $m->quote_name));
                    }

                    $redis->del('write_coinex_lock');
                } else {
                    sleep(3);
                    continue;
                }
