...
class MexcConsumer extends Command
{
    protected $signature = 'start_mexc_consumer';
    protected $description = '消费mexc队列';
...
//                    echo '开始写锁';
            ...
            $redis->expire('write_mexc_lock', 3);
            if ($lock) {
                $match = CurrencyMatch::where('is_mexc', 1)->where('is_enabled', 1)->get();
                foreach ($match as $m) {
                    $redis->rPush('mexc_symbol_list', sprintf('%s%s', $m->currency_name, $m->quote_name));
                }
