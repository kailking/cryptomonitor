...
class LbankConsumer extends Command
{
    protected $signature = 'start_lbank_consumer';
    protected $description = '消费lbank队列';
    ...
    public function handle()
    {
        ...
        $platform = 10;
        while(true){
            $symbol = $redis->lPop('lbank_symbol_list');
            if (!$symbol) {
                //                    echo '开始写锁';
                //拿写锁
                $lock = $redis->setnx('write_lbank_lock', 1);
                $redis->expire('write_lbank_lock', 3);
                if ($lock) {
                    // for($i=0 ;$i<10;$i++){
                    $match = CurrencyMatch::where('is_lbank', 1)->where('is_enabled', 1)->get();
                    foreach ($match as $m) {
                        $redis->rPush('lbank_symbol_list', sprintf('%s%s', $m->currency_name, $m->quote_name));
                    }
                    // }

                    $redis->del('write_lbank_lock');
                }else{
                    sleep(3);
                    continue;
                }
