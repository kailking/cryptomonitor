...
socket = 'wss://www.lbkex.net/ws/V2/'
symbol = []
symbol2 = []
...
def on_message(ws, message):
    ...
    if 'bids' in param:
        if param['bids']:
            ...
            r.set(key1, json.dumps(bids),ex=10)
            # r.expire(key1,60)
    ...
    if 'asks' in param:
        if param['asks']:
            ...
            r.set(key2, json.dumps(asks),ex=10)
            # r.expire(key2,60)
...
def on_open(ws):
    # 获取所有交易对
    cursor.execute('SELECT * FROM currency_match WHERE is_enabled = 1 AND is_lbank = 1 ORDER BY id ASC ' )
    ...
    for row in results:
        ...
        #   symbol.append([currency_name.upper()+quote_name.upper(),5,'0',False])
          ...
          para = json.dumps({
                ...
                "subscribe":"depth",
                "depth":"5",
                "pair":currency_name.lower()+'_'+quote_name.lower()
            })
          ws.send(para)
