...
def on_message(ws, message):
    ...
    if param.has_key('bids') == True:
        if param['bids']:
            ...
            r.set(key1, json.dumps(bids[0:5]),ex=60)
            # r.expire(key1,60)
    ...
    if param.has_key('asks') == True:
        if param['asks']:
            ...
            r.set(key2, json.dumps(asks[0:5]),ex=60)
            # r.expire(key2,60)
...
def on_open(ws):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM currency_match WHERE  is_aex = 1 AND is_enabled = 1 AND (is_huobi = 1 OR is_okex = 1 OR is_gate = 1 OR is_mexc = 1 OR is_biance = 1 OR is_kucoin = 1 OR is_coinex = 1 OR is_lbank = 1 OR is_hotcoin = 1 OR is_ftx = 1 OR is_df = 1)' )
