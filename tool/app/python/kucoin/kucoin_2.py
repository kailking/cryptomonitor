...
async def main():
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM currency_match WHERE is_kucoin = 1 AND is_enabled = 1 AND (is_huobi = 1 OR is_okex = 1 OR is_gate = 1 OR is_mexc = 1 OR is_aex = 1 OR is_biance = 1 OR is_coinex = 1 OR is_lbank = 1 OR is_hotcoin = 1 OR is_ftx = 1) AND currency_name NOT IN (\'WAXP\',\'SUN\',\'GALA\',\'REV\',\'OXEN\',\'BSV\') ORDER BY id ASC LIMIT 200 OFFSET 200' )
